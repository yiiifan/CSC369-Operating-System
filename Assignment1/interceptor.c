...
asmlinkage long intercept_syscall(int syscall, int pid);
asmlinkage long release_syscall(int syscall, int pid);
asmlinkage long monitoring_start(int syscall, int pid);
asmlinkage long monitoring_stop(int syscall, int pid);
...
{
	/* Function: del_pid(pid_t pid) accesses to the pid_list
	 * So, lock before access. */
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	/* i is a flag to record the value of table[reg.ax].monitored */
	int i;
	int j;	
	/* Lock before access the table */
	i = table[reg.ax].monitored;
	/* Unlock after access the table */

	/* j is a flag to record the return value of function:check_pid_monitored() */
	/* When i equals to 1, get the return value of function:check_pid_monitored() */
	if (i == 1) {
		/* Lock before access pid_list */

		j = check_pid_monitored(reg.ax, current->pid);
		/* Unlock after access pid_list */

	}

	/* If monitored = 2, log message,
	 * if monitored = 1 and check_pid_monitored return 1, log message,
	 * else, do not log message. */
	if ((i == 2) || ((i == 1) && (j == 1))) {
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if(cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE){
		/*For the first two commands must be root (see the current_uid() macro).*/
		if(current_uid() != 0){
			return -EPERM;
		}

		if(cmd == REQUEST_SYSCALL_INTERCEPT){
			return intercept_syscall(syscall, pid);
		}

		else if(cmd == REQUEST_SYSCALL_RELEASE){
			return release_syscall(syscall, pid);
		...
		/*pid must be an existing pid except for the case when it's 0*/
		if( (pid_task(find_vpid(pid),PIDTYPE_PID) == NULL && pid != 0) || pid < 0 ){
			return -EINVAL;
		}
		...
		if(cmd == REQUEST_START_MONITORING){
			return monitoring_start(syscall, pid);
		}

		else if(cmd == REQUEST_STOP_MONITORING){
			return monitoring_stop(syscall, pid);
...
asmlinkage long monitoring_start(int syscall, int pid){	
	...
	if(pid == 0){
		...
		/*check whether the pid belong to pid_list */
...
asmlinkage long monitoring_stop(int syscall, int pid){
	...
	if(pid == 0){
		...
		switch(table[syscall].monitored){
			/* pid list is already empty, actually this situation won't exist */
			case 0:			
			spin_unlock(&calltable_lock);
			return -EINVAL;
			...
			/* monitoring all pid, illegal input */
			case 2:
			spin_unlock(&calltable_lock);
			return -EINVAL;
		}
...
static int init_function(void) {
	...
	/*Hijack the exit_group system call (__NR_exit_group) and save the original in orig_exit_group.*/
	...
	/*initializations for bookkeeping data structures*/

	for (temp = 0; temp < NR_syscalls; temp++){  
		INIT_LIST_HEAD(&(table[temp].my_list));
		table[temp].monitored = 0;
		table[temp].intercepted = 0;
		table[temp].listcount = 0;
	}
